...
import static byog.Core.RectangleHelper.isInDeadEnd;
import static byog.Core.RectangleHelper.isWrapped;
...
/*
    ...
    public static TETile[][] worldGenerator(Random RANDOM, TETile[][] world) {
        ...
        for (Room room : rooms) {
            room.randomRemovelWalls(RANDOM, world);
        }
    ...
    private static void removeInnerWalls(TETile[][] world) {
        for (int i = 1; i < world[0].length - 1; i++) {
            for (int j = 1; j < world.length - 1; j++) {
                ...
                if (!isWrapped(new Position(j, i), world)) {
                    continue;
                }
